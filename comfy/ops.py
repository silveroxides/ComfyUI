...
def mixed_precision_ops(quant_config={}, compute_dtype=torch.bfloat16, full_precision_mm=False):
    normalized_config = quant_config
    if normalized_config is None:
        normalized_config = {}
    if isinstance(normalized_config, str):
        try:
            normalized_config = json.loads(normalized_config)
        except Exception:
            logging.warning("Unable to parse quant_config string; falling back to empty config")
            normalized_config = {}

    layer_quant_config = {}
    if isinstance(normalized_config, dict):
        layer_quant_config = normalized_config.get("layers", {}) or {}

    class MixedPrecisionOps(manual_cast):
        _quant_config = normalized_config
...
>>>>>>> master
            ...
            def state_dict(self, *args, destination=None, prefix="", **kwargs):
                ...
                if isinstance(self.weight, QuantizedTensor):
                    ...
                    params = {}
                    block_size = self.weight._layout_params.get('block_size', None)
                    if block_size is not None:
                        params["group_size"] = block_size
                    if self.weight._layout_params.get('is_weight', False):
                        params["asymmetric_layout"] = True
                    if len(params) > 0:
                        quant_conf["params"] = params

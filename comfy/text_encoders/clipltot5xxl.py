...
class ClipLtoT5xxlTEModel(torch.nn.Module):
    def __init__(
        self,
        dtype_t5=None,
        t5_attention_mask=False,
        device="cpu",
        dtype=None,
        model_options={},
    ):
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        print(
            f"t5_out={t5_out}, t5_pooled={t5_pooled}, l_out={l_out}, l_pooled={l_pooled}"
        )
        cut_to = min(l_out.shape[1], t5_out.shape[1])
        t5_out = torch.zeros_like(t5_out)
        lt5_out = torch.cat([l_out[:, :cut_to], t5_out[:, :cut_to]], dim=-1)
        ll_out = torch.nn.functional.pad(
            lt5_out, (0, t5_out.shape[-1] - lt5_out.shape[-1])
        )
        t5_out = torch.cat([ll_out, t5_out], dim=-2)
        t5_pooled = torch.zeros(
            (1, 4096), device=comfy.model_management.intermediate_device()
        )
...
def clipltot5xxl_te(dtype_t5=None, t5xxl_scaled_fp8=None, t5_attention_mask=False):
    class ClipLtoT5xxlTEModel_(ClipLtoT5xxlTEModel):

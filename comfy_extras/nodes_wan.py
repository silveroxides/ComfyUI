...
class WanImageToVideo(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Boolean.Input("tiled_vae", default=False),
    ...
    @classmethod
    def execute(cls, positive, negative, vae, width, height, length, batch_size, tiled_vae, start_image=None, clip_vision_output=None) -> io.NodeOutput:
        ...
        if start_image is not None:
            ...
            if tiled_vae:
                tile_x = round_to_nearest(int(width*0.5), int(32))
                tile_y = round_to_nearest(int(height*0.5), int(32))
                overlap = round_to_nearest(int((tile_x + tile_y)/4), int(32))
                tile_t = round_to_nearest(int((length - 1)*0.5), int(4))
                overlap_t = round_to_nearest(int(tile_t / 8), int(4))
                print(f"tile_x={tile_x}, tile_y={tile_y}, overlap={overlap}, tile_t={tile_t}, overlap_t={overlap_t}")
                concat_latent_image = vae.encode_tiled(image[:,:,:,:3], tile_x=tile_x, tile_y=tile_y, overlap=overlap, tile_t=tile_t, overlap_t=overlap_t)
            else:
